...
for(let urld of data.result) {
  let _url = `https://cors.io/?https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?outSr=4326&forStorage=false&outFields=*&maxLocations=20&address=${encodeURI(urld.main_adddress)}&f=json`;
  $.getJSON(_url, (_data, status) => {
    if(_data.candidates && _data.candidates[0]) {
      let res = _data.candidates[0].location;
      let markerId = maingfx.addMarker(res.y, res.x,
